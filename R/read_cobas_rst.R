...
#'
#' Reads a rst file from a COBAS Liat. Most information is available through middleware,
#' but information on invalid runs is currently not.
#'
...
#'
#' many_results <- list.files("data-folder", full.names = TRUE, pattern = ".rst") %>%
...
read_cobas_rst <- function(file) {
  ...
  report[indented_lines[1]-1] <- paste0(report[indented_lines[1]-1],
                                        report[indented_lines] %>% trimws %>% paste(collapse = ", "))
  report <- report[-indented_lines]
  sep_regex <- " *:(?![0-9]) *"
  report_tbl <- tibble::tibble(x = report) %>%
    tidyr::separate(x, c("var", "value"), sep = sep_regex, fill = "right", extra = "merge")
  ...
  dplyr::bind_rows(report_tbl, hldata_tbl) %>%
    ...
    dplyr::mutate(AnalysisDateTime = readr::parse_datetime(AnalysisDateTime, locale = readr::locale(tz = Sys.timezone())),
           `Tube Exp` = readr::parse_date(`Tube Exp`)) %>%
    dplyr::mutate_at(dplyr::vars(`Approved By`, `Ctrl Exp`, OrderingPhysician), ~ dplyr::na_if(., "N/A")) %>%
    dplyr::mutate(infl_a = detect_result(`Report Results`, "Influenza A"),
           infl_b = detect_result(`Report Results`, "Influenza B"),
           rsv    = detect_result(`Report Results`, "RSV"),
           invalid = stringr::str_detect(`Report Results` %>% stringr::str_to_lower(), "invalid"),
           warning = stringr::str_detect(`Report Results` %>% stringr::str_to_lower(), "warning"))
}
